...
"""
    ...
    # Source 86-93: 任务
    OUTLINE_TASK = """
    1. 设计这一系列短剧的分集大纲，总计拆分成30集（不一定是每个ACT对应10集，需要根据剧情合理分配），每一集130-160字（必须卡在这个范围）。
    2.  完成30集分集大纲生成后，立即对大纲进行全面自检，严格对照以下维度逐一核查、修正，确保大纲符合现象级海外短剧标准，具体检查维度如下：
    a)  逻辑连贯性检查：核查30集剧情整体闭环，无逻辑漏洞、无前后矛盾（人物行为动机、剧情转折、因果关系需合理，符合美国观众认知逻辑）；每集剧情衔接自然，前一集结尾悬念需在后一集合理承接，无断层、无烂尾伏笔；三幕式结构清晰（开端铺垫、发展推进、高潮收尾比例合理），30集剧情节奏张弛有度，避免前期拖沓、后期仓促。
    ...
    3.  完成自检后，修改生成更完善的30集大纲，并划分ACT1/ACT2/ACT3
    """
...
if st.session_state.generated_acts:
    ...
    st.info("请复制您最满意的一个方案（或手动修改）到下方输入框，用于生成 30 集大纲。")
...
# --- Step 3: 分镜脚本 ---
    if st.session_state.outline:
        ...
        with st.expander("📄 查看与手动编辑 30 集大纲", expanded=True):
            # ✅ [核心修改] 将 st.markdown 替换为 st.text_area，使其变为可编辑状态
            # Streamlit 会自动捕捉用户的输入，我们将其重新赋值给 session_state.outline
            edited_outline = st.text_area(
                "请在此处修改或完善大纲内容：",
                value=st.session_state.outline,
                height=400
            )
            # 实时覆盖旧状态：确保后续调用的 gen_script 函数拿到的是人工修改后的最新版本
